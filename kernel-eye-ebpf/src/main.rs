...
/// Ring buffer for streaming telemetry events to user-space (512 KB).
#[map]
...
//    2. If the target task's TGID matches the agent and the caller is
//       NOT the agent itself → DENY (-EPERM) regardless of signal number.
//    3. Otherwise → allow.
...
fn try_task_kill(ctx: LsmContext) -> Result<i32, i32> {
    ...
    // 3. If someone is trying to kill the agent → block ANY signal
    //    (only the agent may send signals to itself).
    if target_tgid == agent_tgid {
        let caller_tgid = bpf_get_current_pid_tgid() as u32;
        if caller_tgid == agent_tgid {
            return Ok(0); // self‑signaling is allowed
        }
...
fn try_task_free(ctx: LsmContext) -> Result<i32, i32> {
    ...
    // Emit a lightweight process‑exit event (optional, currently silent).
